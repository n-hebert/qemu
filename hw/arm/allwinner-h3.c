...
void allwinner_h3_bootrom_setup(AwH3State *s, BlockBackend *blk)
{
    ...
    rom_add_blob("allwinner-h3.bootrom", buffer, rom_size,
                  rom_size, s->h3_memmap[AW_H3_DEV_SRAM_A1],
...
static void allwinner_h3_realize(DeviceState *dev, Error **errp)
{
    ...
    memory_region_init_ram(&s->sram_c, OBJECT(dev), "sram C",
                            44 * KiB, &error_abort);
    memory_region_add_subregion(get_system_memory(), s->h3_memmap[AW_H3_DEV_SRAM_A1],
                                &s->sram_a1);
    memory_region_add_subregion(get_system_memory(), s->h3_memmap[AW_H3_DEV_SRAM_A2],
                                &s->sram_a2);
    memory_region_add_subregion(get_system_memory(), s->h3_memmap[AW_H3_DEV_SRAM_C],
                                &s->sram_c);
